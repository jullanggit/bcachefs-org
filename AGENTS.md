...
- `bch2_reconcile_completed_kick()` only means the requested reconcile pass drained. Shrink's final journal/key-cache flush may still need the reconcile thread to reach its idle state, otherwise cached reconcile paths can keep key-cache journal pin flushing stalled long after the kick completed.
