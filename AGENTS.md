...
- Live foreground IO can keep a shrinking tail non-empty for much longer than a couple of reconcile kicks even when enough space exists. When deciding that shrink is truly stuck, key the ENOSPC decision off stalled tail progress rather than a small fixed retry count.
