...
- In the continuous harness, background stress workers must be stopped before offline `fsck`/remount checks. A drained op queue is not enough if a long-lived worker is still mutating the mounted filesystem.
...
- Continuous-harness checks that unmount the filesystem, such as offline `fsck` and remount validation, must only run once the manager has drained in-flight async operations. Running them while a resize is still active turns the check itself into interference.
#### ssh
