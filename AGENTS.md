...
- If a guest-side Rust property test runs multiple `proptest` cases against one mounted filesystem, each case must restore the starting topology before returning. Otherwise later cases and shrink reruns inherit prior device membership changes and the minimized reproducer stops describing one isolated history.
