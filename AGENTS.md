...
- `bcachefs set-fs-option` online writes directly through sysfs. In the current userspace tree, `sysfs_write_str()` ignores write errors, so option changes that the kernel rejects can appear to succeed unless the caller re-reads sysfs or otherwise verifies the result.
- In the continuous harness, periodic `bcachefs fsck -n` checkpoints run online every fixed number of completed actions instead of unmounting for offline `fsck`. End-of-run cleanup may still stop workers before teardown, but routine health checks should not destroy the live state they are trying to exercise.
- If online `bcachefs fsck -n` turns out to be too noisy while workers are actively mutating the filesystem, defer the checkpoint until the next quiescent point instead of forcing an exact-action cadence through obvious checkpoint interference.
- The continuous harness assumes the test filesystem stays mounted for the entire case. Do not carry an explicit mounted/unmounted model state there; if a live query finds the filesystem gone, treat that as an immediate failure instead of a recoverable branch in the harness logic.
- In the continuous harness, keep the device-label namespace independent from the target kinds. Device labels should be a small arbitrary set such as `a`/`b`/`c`, while target kinds remain `foreground_target`, `background_target`, and `promote_target`. That lets the manager exercise cases where different targets resolve to different device groups instead of collapsing everything onto foreground/background naming.
- For the continuous Rust harness, prefer a single manager loop that launches async operations and reasons from live observations over a sequential `proptest` transition model. Resizes need real overlap so newer requests can supersede older ones.
...
- In the continuous harness, `RemoveDevice` is only a hard-success assertion in simple runtime configurations. Once labels/targets or background workers are active, keep remove operations for stress coverage but treat the exact success/failure outcome as ambiguous unless the current state is simple enough to reason about confidently.
- For those ambiguous `RemoveDevice` failures, only assert invariants/safety. A failed live remove can leave `fs usage` reporting transitional read-only or evacuating states with unstable device names such as `dev-0`, so exact post-failure topology assertions are too strong there.
...
- The first remaining tail bucket can stay unchanged while shrink is still making space deeper in the tail. Liveness heuristics should watch aggregate tail-backpointer progress as well as the head bucket, slice shrink waits so one long reconcile kick cannot hide that progress behind minute-scale stalls, and only count no-progress full rescans after the journal stayed quiet for that pass. Journal churn means foreground IO or reconcile is still changing the blocker set.
- The current journal-quiescence check is filesystem-global. Unrelated metadata-writing IO can keep advancing `journal_cur_seq()` and suppress shrink's stalled-pass counter indefinitely. If this becomes a problem, the right follow-up is a shrink-local churn signal rather than tightening the global journal heuristic.
